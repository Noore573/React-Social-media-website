...
const CreateForm = () => {
  ...
  const schema = yup.object().shape({
    ...
    media: yup.mixed().required("Insert media file"),
  });
  const {
    register,
    handleSubmit,
  ...
  return (
    <form className="createform" onSubmit={handleSubmit(handleUploadAndCreatePost)}>
      <input id="title" placeholder="Title..." {...register("title")} />
      <p className="errorMsg">{errors.title?.message}</p>
      <textarea placeholder="Description" {...register("description")} />
      <p className="errorMsg">{errors.description?.message}</p>
      <input type="file" accept="image/jpeg, video/*" onChange={handleMediaChange} />
